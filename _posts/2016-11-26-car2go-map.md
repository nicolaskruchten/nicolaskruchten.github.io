...
When I was done, it turned out that the Python code required to make such a map, including making the circles pop up  an hour-of-day distribution chart when clicked, was only about 30 lines long, which was a pleasant surprise! I've posted [a version with side-by-side commentary](http://nicolas.kruchten.com/car2go_map/) but here it is without comments in order to appreciate its conciseness:
